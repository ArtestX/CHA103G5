...
<body>
    ...
    <br>
    <c:choose>
        <c:when test="${not empty oneOrderDetail}">
            <table style="width:50%; text-align:center;">
                ...
                <tr>
                    <td>${oneOrderDetail.orderDetailNo}</td>
                    <td>${oneOrderDetail.orderTableNo}</td>
                    <td>${oneOrderDetail.productNo}</td>
                    <td>${oneOrderDetail.quantity}</td>
                    <td>${oneOrderDetail.productPrice}</td>
                    <td>
                        <form action="${pageContext.request.contextPath}/orderDetailServlet" method="post">
                            <input type="hidden" name="action" value="edit" />
                            <input type="hidden" name="orderDetailNo" value="${oneOrderDetail.orderDetailNo}" />
